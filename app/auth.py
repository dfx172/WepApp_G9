...
def register():
    try:
        ...
        if request.method == 'POST':
            ...
            print("antes de validar si se escribio usuario")
            if not username:
                error = 'Username is required.'
                flash(error)
                return render_template('auth/register.html')
            print("antes de validar si usuario es valido")
            if not utils.isUsernameValid(username):
                error = "Username should be alphanumeric plus '.','_','-'"
                flash(error)
                print("usuario invalido")
                return render_template('auth/register.html')
            print("antes de validar si se escribio contraseña")
            if not password:
                error = 'Password is required.'
                flash(error)
                return render_template('auth/register.html')
            print("antes de validar si el usuario esta repetido")
            if db.execute('SELECT username FROM user WHERE username = ?', (username,)).fetchone() is not None:
                ...
                return render_template('auth/register.html')
            print("despues de validar si el usuario esta repetido")
            # (not email)
...
def confirm():
    try:
        ...
        if request.method == 'POST':
            ...
            print("se mira si attempt tiene algo")
            print("attempt: ", attempt['id'])
            if attempt is not None:
                print("attempt tenia algo")
                db.execute(
...
def change():
    try:
        ...
        if request.method == 'GET':
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT userid from forgotlink WHERE challenge=? AND state=?', (
...
def forgot():
    try:
        ...
        if request.method == 'POST':
            email = request.form['email']
            print("antes de comprobar el email")
            if ((not email) or (not utils.isEmailValid(email))):
                error = 'Email Address Invalid'
                flash(error)
                return render_template('auth/forgot.html')
            print("antes de hacer la conexion")
            db = get_db()
            user = db.execute(
                'SELECT * from user WHERE email=?', (email,)
            ).fetchone()
            print("despues de hacer la conexion")
            if user is not None:
                number = hex(random.getrandbits(512))[2:]
